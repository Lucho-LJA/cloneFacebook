class FriendshipsController < ApplicationController
  ...
  def create
    return if current_user.id == params[:board_id] # Disallow the ability to send yourself a friend request
    # Disallow the ability to send friend request more than once to same person
    return if friend_request_sent?(User.find(params[:board_id]))
    # Disallow the ability to send friend request to someone who already sent you one
    return if friend_request_recieved?(User.find(params[:board_id]))
    params[:board_id]
    @user = User.find(params[:board_id])
    @friendship = current_user.friend_sent.build(sent_to_id: params[:board_id])
    if @friendship.save
  ...
  def accept_friend
    @friendship = Friendship.find_by(sent_by_id: params[:board_id], sent_to_id: current_user.id, status: false)
