class LikesController < ApplicationController
  ...
  def already_liked?(type)
    ...
    if type == 'user'
      result = Like.where(user_id: current_user.id,
                          user_like_id: params[:user_like_id]).exists?
    end
    result
  end
  ...
  def dislike(type)
    ...
    @like = Like.find_by(user_like_id: params[:user_like_id]) if type == 'user'
    return unless @like
